...
def index():
    if request.method == 'POST':
        ...
        # connecting the database called crawlerDB
            db = dbConn['crawlerDB']
            print("database running")
            reviews = db[searchString].find({})# searching the collection with the same name)
            if reviews.count() > 0:
                return render_template('return.html', reviews=reviews)
            else:
                searchString=request.form['content'].replace(" ", "")
